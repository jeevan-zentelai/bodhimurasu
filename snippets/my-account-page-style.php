...
// Pass redirect_to through login form
function bodhi_login_form_redirect() {
    if (isset($_GET['redirect_to']) && !empty($_GET['redirect_to'])) {
        echo '<input type="hidden" name="redirect" value="' . esc_url($_GET['redirect_to']) . '">';
    }
}
add_action('woocommerce_login_form', 'bodhi_login_form_redirect');
add_action('woocommerce_register_form', 'bodhi_login_form_redirect');

// After login redirect to checkout
function bodhi_login_redirect($redirect, $user) {
...
add_filter('woocommerce_login_redirect', 'bodhi_login_redirect', 10, 2);

// After register redirect to checkout
function bodhi_register_redirect($redirect) {
    if (isset($_POST['redirect']) && !empty($_POST['redirect'])) {
        return esc_url_raw($_POST['redirect']);
    }
    if (isset($_GET['redirect_to']) && !empty($_GET['redirect_to'])) {
...
add_filter('woocommerce_registration_redirect', 'bodhi_register_redirect');

// Force login before cart/checkout
function bodhi_force_login_checkout() {
    if (is_user_logged_in()) return;
    if (is_cart() || is_checkout()) {
        $checkout_url = wc_get_checkout_url();
        $login_url    = wc_get_page_permalink('myaccount') . '?redirect_to=' . urlencode($checkout_url);
        wp_redirect($login_url);
        exit;
    }
}
add_action('template_redirect', 'bodhi_force_login_checkout');

// Force login before add-to-cart
function bodhi_force_login_add_to_cart() {
...
add_action('template_redirect', 'bodhi_force_login_add_to_cart', 5);